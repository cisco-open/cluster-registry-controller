...
func (r *ClusterReconciler) watchLocalClustersForConflict(b *builder.Builder) {
	b.Watches(&source.Kind{Type: &clusterregistryv1alpha1.Cluster{
		...
		ToRequests: handler.ToRequestsFunc(func(object handler.MapObject) []ctrl.Request {
			...
			clusters, err := GetClusters(r.GetContext(), r.GetManager().GetClient())
			if err != nil {
...
func (r *ClusterReconciler) watchClusterRegistrySecrets(b *builder.Builder) {
	b.Watches(&source.Kind{Type: &corev1.Secret{
		TypeMeta: metav1.TypeMeta{
			Kind:       "Secret",
		...
		ToRequests: handler.ToRequestsFunc(func(object handler.MapObject) []ctrl.Request {
			reqs := make([]reconcile.Request, 0)
			if secret, ok := object.Object.(*corev1.Secret); ok {
