...
func main() {
	...
	err = vfsgen.Generate(
		ZeroModTimeFileSystem{
			http.Dir(path.Join(getRepoRoot(), "deploy/charts/cluster-registry"))},
		vfsgen.Options{
			Filename:     path.Join(getRepoRoot(), "static/charts/cluster_registry/chart.gogen.go"),
			PackageName:  "cluster_registry",
			VariableName: "Chart",
		})
	if err != nil {
		panic(err)
